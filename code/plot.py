...
def plot_score_comparison_2D(df, value, cluster1, cluster2, percentile1=99,percentile2=99, 
                             ...
                             title2=None,if_line=True,save1=None,save2=None,links=None,goi_size=10):
    """
    ...
    thre2 = np.percentile(piv[cluster1][piv[cluster1].values<0].values, 100-percentile1)
    thre3 = np.percentile(piv[cluster2].values, percentile2)
    print(thre1,thre2,thre3)
    
    gois_ = np.union1d(goi1, goi2)
    if select == 'union':
        gois = np.union1d(gois_, goi3)
    if select == 'intersect':
        gois = np.intersect1d(gois_, goi3)
    ...
    # plot another figure without the unselected points
    if plot_de_gene:

        # find the intersection of the differential TFs and the selected TFs
        goi1_ = piv[piv[cluster1] > 0].index
        goi2_ = piv[piv[cluster1] < 0].index
        intersect_goi = np.union1d(np.intersect1d(goi1_,pos_gene_tf),np.intersect1d(goi2_,neg_gene_tf))
        ...
        for goi in gois:
            x, y = piv.loc[goi, cluster1], piv.loc[goi, cluster2]
            if goi in intersect_goi:
                plt.scatter(x, y, c="none", edgecolor="black")
                _plot_goi(x, y, goi, {}, scatter=False, x_shift=x_shift, y_shift=y_shift,color='red',goi_size=goi_size)
            else:
                plt.scatter(x, y, c="none", edgecolor='black')
                _plot_goi(x, y, goi, {}, scatter=False, x_shift=x_shift, y_shift=y_shift,color='#1f77b4',goi_size=goi_size)
